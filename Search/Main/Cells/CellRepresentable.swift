...
//
//    case resultSection([ResultCell])    // '검색' 버튼 눌렀을때 결과 뿌려주는 셀
//    case noResultsCell
//}

enum ResultCell {
    case appIconCell(AppIconViewModel)
    case landScapeScreenShotCell(LandScapeViewModel)          // 스크린샷이 1장(landscape 모드)
    case portaitScreenShotCell(PortaitViewModel)              // 스크린샷 2장 이상 (portrait 모드)
}
